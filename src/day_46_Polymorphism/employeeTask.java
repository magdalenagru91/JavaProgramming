...
import day_43_Abstraction.employeeTask.Person;
    ...
    public static void main(String[] args) {
        ...
        for (Employee each : scrumMembers) {
            if (each instanceof Tester) {
                testers.add((Tester) each); //we put bigger into smaller, so employee is bigger, tester smaller, we wanna put employee into tester
            }
        }

        int maxSalary = (int) testers.get(0).getSalary();
        String testerMaxSalary = "";

        for (Tester tester : testers) {
                if (maxSalary < tester.getSalary()) {
                    maxSalary = (int) tester.getSalary();
                    testerMaxSalary=tester.getName();
                }
            }


            System.out.println("Tester with max salary is: " + maxSalary + " " + testerMaxSalary);
 ...
 */
            ...
            int maxSalaryDeveloper = (int) developers.get(0).getSalary();
            String developerMaxSalary = "";

        for (Developer developer : developers) {
            if (maxSalaryDeveloper<developer.getSalary()){
                maxSalaryDeveloper= (int) developer.getSalary();
                developerMaxSalary=developer.getName();
            }
        }


        System.out.println("Developer with max salary is: "+maxSalaryDeveloper + " " + developerMaxSalary);

        /*
        for (int i = 0; i < developers.size(); i++) {
            developers.removeIf(p -> p.getSalary().);
        }

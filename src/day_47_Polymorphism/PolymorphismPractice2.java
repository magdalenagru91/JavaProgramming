...
public class PolymorphismPractice2 {
    ...
    public static void main(String[] args) {
        ...
        for (Employee employee : employees) {       //it references to all the child objects in the employee array
        ...
        for (Employee scrumMember : scrumMembers) {
            if (scrumMember instanceof Tester){
                testers.add((Tester)scrumMember);           //I need casting cause type is Employee (supertype)
                                                            // - you cannot apply supertype to child directly
                                                            // - to Tester
            }
        ...
        Developer developerWithMaxSlary = developers.get(0);
        ...
        for (Developer developer : developers) {
            if (developer.getSalary()>developerWithMaxSlary.getSalary()){
                developerWithMaxSlary=developer;
            }
        }
        System.out.println("developerWithMaxSlary = " + developerWithMaxSlary);
